...
;If a file has 'Q' as 2nd char. of filetype, prints msg
;saying if file doesn't print correctly, use TYPESQ.
;
...
;
;07/07/71 Ver 5.0 - Added test for possible squeezed file.
;	  (KBP)
;
...
START2:	LXI	H,0
	...
	CALL	ILPRT	;PRINT:
	DB	'MLIST ver 5.0 - Multiple file lister',CR,LF
...
;
	...
	ANI	7FH	;STRIP CP/M 2.x ATTR
	CPI	'O'	;AN 'O'?
	JNZ	SQCHK	;IF NOT IT'S OK TO PRINT
	DCX	H
...
OBJCHK:	CPI	'J'	;WAS LAST CHAR AN 'J' ?
	JNZ	SQCHK	;IF NOT, OK TO LIST
	DCX	H
	MOV	A,M	;MIGHT BE '.OBJ', CHK NEXT CHR
	ANI	7FH	;STRIP CP/M 2.x ATTR
	CPI	'B'	;IS IT A 'B'?
	JNZ	SQCHK	;IF NOT, LIST
	DCX	H
	MOV	A,M	;WAS, CHK FIRST CHAR
	ANI	7FH	;STRIP CP/M 2.x ATTR
	CPI	'O'	;'O' AS IN '.OBJ'?
	JNZ	SQCHK	;IF NOT, PRINT THE FILE, IF SO
	CALL	ILPRT	;PRINT:
...
;
SQCHK:	LDA	FCB+10	;GET SECOND CHAR OF FILETYPE
	ANI	7FH	;STRIP ATTRIBUTE
	CPI	'Q'	;THIS MAY BE SQUEEZED FILE
	JNZ	READLP	;NOT SQUEEZED
	CALL	ILPRT	;PRINT:
	DB	'NOTE: If this file doesn''t '
	DB	'print correctly, try TYPESQ.',CR,LF
	DB	'Most files with "Q" as 2nd char. '
	DB	'of filetype are squeezed.',CR,LF,CR,LF,0
;
