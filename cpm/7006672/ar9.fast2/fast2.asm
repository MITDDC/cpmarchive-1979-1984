  TITLE		'FAST - CP/M SPEED-UP BY DISK I/O BUFFERING'
; FILENAME	FAST.ASM
; AUTHOR	Robert A. Van Valzah    12/25/78
; LAST REVISED	Bruce R. Ratoff	5/2/80
; REASON	CP/M VERSION 2.X COMPATIBILITY
;
;
;  \\\\\\\\\\\\\\\\\                   /////////////////
;   >>>>>>>>>>>>>>>>>  E Q U A T E S  <<<<<<<<<<<<<<<<<
;  /////////////////                   \\\\\\\\\\\\\\\\\
;
BASE	EQU	$	;ORG FOR THIS ASSEMBLY OF FAST (100H IF $+R WAS USED)
MAJVERS	EQU	1	;FAST MAJOR VERSION NUMBER
MINVERS	EQU	01	;FAST MINOR VERSION NUMBER
;
...
;
	ORG	0	;MAC $+R OPTION WILL TAKE CARE OF HIGH ORIGIN
...
;
	ORG	(($-1) OR 255) + 1 - BASE	;ORG TO PAGE BOUNDRY
...
FSETDSK:
	...
	INX	H	;(SEE IF WE HAVE ITS .DPH)
	MOV	H,M	;SET UP TO RETURN .DPH
	MOV	L,A
	ORA	H	;IS .DPH(DRIVE) NONZERO?
	SHLD	REQDPH
	RNZ		;YES - DONE WITH SELECTION
	MOV	C,E	;SET UP FOR SELECT
	CALL	CSETDSK	;SELECT THE SPECIFIED DRIVE
	XCHG		;SAVE .DPH RETURNED BY CSETDSK
	LHLD	REQDSK	;GET DRIVE #
	MVI	H,0
	DAD	H	;TIMES 2
	LXI	B,ONLINE;PLUS BASE OF ONLINE VECTOR
	DAD	B
	MOV	M,E	;STORE .DPH
	INX	H
	MOV	M,D
	XCHG
	SHLD	REQDPH	;SAVE LOCALLY
	CALL	CHOME	;HOME THE DRIVE
	LHLD	DIRADR	;AND READ THE DIRECTORY IF IT IS BUFFERED
	CALL	RDDDB
	LHLD	REQDPH	;RETURN DPH TO CALLER
	RET
;
; FAST READ SECTOR
; EXTERNALLY REFERENCED ROUTINE.
...
REQDMA	DW	0FFFFH
;
ONLINE	DW	0, 0, 0, 0 ;DRIVE ON LINE VECTOR (STORES .DPH IN VERS 2)
;
	DS	10H	;AT LEAST AS MUCH STACK AS CCP
;
	ORG	(($-1) OR 255) + 1 - BASE	;ORG TO NEXT PAGE BOUNDRY
STACK:
;
CODELEN	EQU	$-BASE
;
	ORG	CODELEN-BASE	;INFORM OPERATOR OF CODE LENGTH
	END	FASTNTRY
