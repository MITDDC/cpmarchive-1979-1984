;
;		   FROMHARD.ASM ver 1.2
;		 by Keith Petersen, W8SDZ
;	     	     (revised 9/26/80)
;
;This program will transfer files from CP/M on hard disk
;to CP/M on floppy disk.  (It can also be used for transfers
;from 8" floppy to 5-1/4" minifloppy.)
;	
...
; 2 - Cold boot the floppy CP/M.  Log into the drive you will
;     be transferring files to, then run this program.
;
;COMMANDS:
;	FROMHARD filename.type	 ;gets file from default drive
;	FROMHARD B:filename.type ;gets file from B:
;	FROMHARD *.*		 ;gets all files
;	FROMHARD B:*.*		 ;gets all files from B:
;
;All normal ambiguous file names are allowed. Files written to
;the floppy disk will go to the default drive.
;
FALSE	EQU	0
TRUE	EQU	NOT FALSE
;
...
;
SRC$BDOS EQU	CCP+806H   ;SENDING BDOS VECTOR
;
...
BDOS	EQU	0005H	;THE RECEIVING BDOS VECTOR
...
SIGNON:	LXI	SP,STACK  ;SET STACK POINTER
	CALL	ILPRT	;PRINT:
	DB	CR,LF,'HARD DISK to FLOPPY',CR,LF
