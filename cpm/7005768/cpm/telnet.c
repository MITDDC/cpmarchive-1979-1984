...
#define SPECIAL 0x1f	/* The character you type to
			   signal a Telnet command (should
			   be obscure...I use a null)	*/
#define CPMSIZE 64	/* Amount of RAM used by CP/M in
			   your system (in K)		*/
#define CLEARS "esc K"	/* string which, when output to
...
#define KBSTAT	0x22	/* Keyboard status port 	*/
#define KBIMASK 0x04	/* Keyboard status input mask	*/
#define KBAHI	1	/* 1 if kb status is active hi	*/
#define KBDATA	0x20	/* Keyboard data port		*/
...
#define MIMASK	0x01	/* Modem input status mask	*/
#define MOMASK	0xf0	/* Modem output status mask	*/
	...
	if (kbready()) {
	  ...
	  if (c != SPECIAL) {
		...
		case 'B':  free = free--;
			   cptr = cptr--;
			   if (rflag) rclose();
			   else printf("yo got no file");
			   goto lf;
		case 'E':  free = free--;
			   cptr = cptr--;
			   if (rflag) rdump(1);
			   else printf("weah yo file at");
			   goto lf;
	  ...
	  default: printf("BDS Telnet commands are:\n");
		   ...
		   printf("e: strip last char and dump buffer\n");
		   printf("c: dump buffer and close output file\n");
		   printf("b: strip last char, dump & close file\n");
...
/*
	...
	MODIFACTION BY B.DOBYNS 10/09/80:
	NO OUTPUT TO LPTR EVER!!
*/
