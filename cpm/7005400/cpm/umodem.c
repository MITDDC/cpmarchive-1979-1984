...
/*  This flag should be set to TRUE if Page Mode is supported on your
 *  UNIX system.  If it is supported, make sure that TIOCSSCR is the
 *  correct name for Page Mode engagement.
 */
#define	     PAGEMODE	FALSE	; Page Mode (TIOCSSCR) not supported
...
char **argv;
{
	...
	if (argc < 3 || *argv[1] != '-')
	{  printf("\nUsage:  \n\tumodem ");
		printf("-[rb!rt!sb!st][p][l][1][a][m][d][y][7][4]");
		printf(" filename\n");
	   printf("\n");
	   printf("\n\trb <-- Receive Binary");
	   printf("\n\trt <-- Receive Text");
